...
public class PessoaController {
	...
	public ResponseEntity<?> findById(@PathVariable("id") Long idPessoa) {
		...
		if(pessoa == null)
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Não encontrou");
			//return ResponseEntity.status(HttpStatus.NOT_FOUND).body(MessagesLoader.getMessage("message.pessoa_nao_encontrada"));
	
		return ResponseEntity.status(HttpStatus.OK).body(pessoa);
	}
	
	@GetMapping(path = "/buscar-pessoa",
		     	produces = {MediaType.APPLICATION_JSON_VALUE,
			        		MediaType.APPLICATION_XML_VALUE,
							MediaType.APPLICATION_YAML_VALUE},
		     	consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<PessoaDTO>> find(@RequestBody(required = false) PessoaDTO pessoaDTO) {
	...
	@PutMapping(path = "/{id}",
				produces = {MediaType.APPLICATION_JSON_VALUE,
							...
							MediaType.APPLICATION_YAML_VALUE},
		        consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<PessoaDTO> alteraPessoa(@PathVariable("id") Long idPessoa, @RequestBody PessoaDTO pessoaDTO) {
